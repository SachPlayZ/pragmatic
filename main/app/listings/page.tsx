...
import ListCard from "@/components/MajorUI/ListCard";
import ListForm from "@/components/MajorUI/ListForm";
import { useEffect, useState } from "react";
import { GetAllProperties } from "@/components/functions/GetAllProperties";
// import CoolOverlayModal from "@/components/MajorUI/CoolOverlayModal";
...
export default function Listing() {
  ...
  const fetchComparisonData = async (
    ...
    try {
      const response = await fetch(
        `${process.env.VITE_PUBLIC_BACKEND_URL}/getComparison`,
        {
          method: "POST",
          headers: {
  ...
  useEffect(() => {
    const fetchProperties = async () => {
      ...
      try {
        const response = await fetch(
          `${process.env.VITE_PUBLIC_BACKEND_URL}/property`
        );
        const data = await response.json();
        setProperties(data);
      } catch (error) {
